...
//        JOptionPane.setDefaultLocale(locale);
        ...
        if(lockInstance(dir + "/mw_instance.lock"))
        {
            if(args.length == 0)
            {
...
//                    break;
     ...
     {
         if(System.getProperty("user.dir").endsWith("NetBeansProjects\\Marketwatch"))
             return;
         ...
         if(!System.getProperty("user.dir").endsWith("Marketwatch"))
        {
            if(new SystemInfo().getOperatingSystem().getFamily().equals("Windows"))
            {
